...
%     end     
    ...
    variable_set = [...%                     2.5; %     robot.n_cs = 1.65; 1.9; 
                    2.5; %     robot.n_cb = 1.8;         2.35; %     robot.n_rb = 2.65;
                    10; %     robot.Dhr = 7e6; %                     0.7; %     human.limit_def = 0.9;
                    ];
...
% % % % % % % % % % Fitting Force Data into Gaussians for each Velocity % % % % % % % % 
        maxTime3 = 24; maxTime2 = 80; maxTime1 = 80;
        
        maxTime7 = 30; maxTime6 = 80; maxTime5 = 80;
        maxtimeVec = [80;
                      80;
                      24;
                      24;
        ...
        for iTest=1:8
           test{iTest}.end = find(timeSet(:,iTest)>=maxtimeVec(iTest),1);    
           test{iTest}.Force = F_set(1:test{iTest}.end,iTest);
           test{iTest}.time = timeSet(1:test{iTest}.end ,iTest);
           if iTest < 3
            [test{iTest}.fitresult, test{iTest}.gof] = FitForceLegs3(test{iTest}.time, test{iTest}.Force);
           else
            [test{iTest}.fitresult, test{iTest}.gof] = FitForceLegs(test{iTest}.time, test{iTest}.Force);   
           end
           F_fit = cat(2,F_fit,test{iTest}.fitresult(timeSet(:,1)));
        end
