...
%     weight_samples = [5:20:205];
    ...
    material_names = {'EPDM',   'HDPE',   'PP',   'ABS',  'PBT',   'Nylon',    'PC',  'CF' };%'Al-7075'};
    material_modulus = [6.73*1e6 ; 1e9; 1.325e9; 1.4*1e9; 1.93e9; 3.3*1e9;   3.4*1e9; 10.0*1e9;];%368*1e9; ];
    material_poisson = [0.5 ;     0.46;    0.42;   0.29;   0.39;    0.41;     0.34;   0.30;];% 0.35;  ];
    
    X_variable = material_modulus./1e9;
    Variable2_label = 'Elastic modulus (GPa)';
...
%     qolo_robot.bumper_radius = radius_samples(i_radius);
    ...
    for k_material=1:length(X_variable)
...
%         qolo_robot.bumper_radius = radius_samples(k_curvature);
        qolo_robot.elastic_mod_robot_cover = material_modulus(k_material);
        qolo_robot.poisson_ratio_cover = material_poisson(k_material);
        ...
        for j_test=1:length(vel_samples)
            ...
            service_sim{j_test,k_material}.colModel = contact_simulation(state,robot,human,method);
            max_energy_density(j_test,k_material) = service_sim{j_test,k_material}.colModel.max_energy_density; 
            max_compressive(j_test,k_material) = max(service_sim{j_test,k_material}.colModel.compressive_stress);
            max_tensile(j_test,k_material) = service_sim{j_test,k_material}.colModel.max_tensile;
            max_Fext(j_test,k_material) = -min(service_sim{j_test,k_material}.colModel.Fext);
            max_acc(j_test,k_material) = max(service_sim{j_test,k_material}.colModel.acc);
            [HIC_tab(j_test,k_material),~] = HIC15_criteria(service_sim{j_test,k_material}.colModel.timeVec,...
                                                        service_sim{j_test,k_material}.colModel.acc,1,false);            
        end
...
%                 find(service_test{j_test}.data.time>minTime,1):find(service_test{j_test}.data.time>maxTime,1);
    ...
    X_variable(find(mean_max_norm<0.30,1))
    mean_max_norm(find(mean_max_norm<0.30,1))
    
    X_variable(find(mean_hic_norm<0.30,1))
    mean_hic_norm(find(mean_hic_norm<0.30,1))
    ...
    %% Head Impact Simulation Plots:
       ...
       labelOrientation = 'vertical';
        figName = 'SimE-Force_peak-child-head';
...
%         end
        ...
        plotNpairedData(nfig,[operational_vel',... 
                            ...
                            '-',figName,plegends,...
                            'Operational velocity at impact (m/s)','Max Contact Force (kN)'...
                            ,true,figPath,figureFormat...%                             ,AxisPlots...
                            );
        hold on;
        plot(1.0 - pedestrian_vel,max(data_filtered.test_5.impact.Fx).*1e-3,'ok','lineWidth',LinesWidths,'MarkerSize',14);
        plot(1.5 - pedestrian_vel,max(data_filtered.test_6.impact.Fx).*1e-3,'ok','lineWidth',LinesWidths,'MarkerSize',14);
        plot(3.1 - pedestrian_vel,max(data_filtered.test_7.impact.Fx).*1e-3,'ok','lineWidth',LinesWidths,'MarkerSize',14);
        ...
        nfig = nfig+1;
        figName = 'SimE-Acceleration-peak-child-head';
        plegends = material_names;
        minTime = 0;
        maxTime = method.time;
        AxisPlots = [minTime maxTime -100 7000];
        plotNpairedData(nfig,[operational_vel',... 
                            ...
                            '-',figName,plegends,...
                            'Operational velocity at impact (m/s)','Max Acceleration (g)'...
                            ,true,figPath,figureFormat...%                             ,AxisPlots...
                            );
        hold on;
        plot(1.0 - pedestrian_vel,Q3.PeakAcc(5),'ok','lineWidth',LinesWidths,'MarkerSize',12);
        plot(1.5 - pedestrian_vel,Q3.PeakAcc(6),'ok','lineWidth',LinesWidths,'MarkerSize',12);
        plot(3.1 - pedestrian_vel,Q3.PeakAcc(7),'ok','lineWidth',LinesWidths,'MarkerSize',12);
        PicSize = [10 10 780 780];
        hLegend = legend(...
                  plegends, ...
                  'FontName',Fonts,...
        ...
        plotNpairedData(nfig,[operational_vel',... 
                            ...
                            '-',figName,plegends,...
                            'Operational velocity at impact (m/s)','HIC'...
                            ,true,figPath,figureFormat...%                             ,AxisPlots...
                            );
        hold on;
        plot(1.0 - pedestrian_vel,Q3.HIC15_robot(5),'ok','lineWidth',LinesWidths,'MarkerSize',14);
        plot(1.5 - pedestrian_vel,Q3.HIC15_robot(6),'ok','lineWidth',LinesWidths,'MarkerSize',14);
        plot(3.1 - pedestrian_vel,Q3.HIC15_robot(7),'ok','lineWidth',LinesWidths,'MarkerSize',14);
        PicSize = [10 10 780 780];
        hLegend = legend(...
                  plegends, ...
                  'FontName',Fonts,...
...
%         contourf(X_variable,vel_samples,Zcontour,'--');
        ...
        contourf(X_variable,operational_vel,Zcontour,zlevs,'edgecolor','none');
        hold on
        contour(X_variable,operational_vel,Zcontour,zindex,'--k','LineWidth',3)
        colormap(customCMap);
        cb = colorbar('southoutside');
        set(get(cb,'label'),'string','Peak Acceleration [g]');
        Xlabel=Variable2_label;
        Ylabel='Operational velocity (m/s)';
        figName = 'SimE-Heatmap-Max-acceleration-Child-head';
        set(gcf, 'name', figName);
        set(gcf, 'Position', [10 10 880 880]);
        ...
        set([hXLabel, hYLabel]  , ...
                ...
                'FontSize',  FontSizes,...
                'color',     [0 0 0]);
        AxisRange = [X_variable(1), X_variable(end), operational_vel(1), operational_vel(end)];
        axis(AxisRange);
        hold on;
%         plot([X_variable(6)],[1.0],'ok','lineWidth',LinesWidths,'MarkerSize',14);
%         plot([X_variable(6)],[1.5],'ok','lineWidth',LinesWidths,'MarkerSize',14);
%         plot([X_variable(6)],[3.1],'ok','lineWidth',LinesWidths,'MarkerSize',14);
        for i_material=1:length(X_variable)
            plot([X_variable(i_material) X_variable(i_material)],[AxisRange(3), AxisRange(4)],'-.',...
                                'Color',[0.3 0.3 0.3],'lineWidth',1.2); %'MarkerSize',10);
        end
    %     hold on;
...
%         contourf(X_variable,vel_samples,Zcontour,'--');
        ...
        Ylabel='Operational velocity (m/s)';
        figName = 'SimE-Heatmap-HIC-Child-head';
        set(gcf, 'name', figName);
        set(gcf, 'Position', [10 10 880 880]);
        set(gca,'FontName',Fonts,...
                'FontSize', FontSizes,...
...
%         set(get(cb,'label'),'string','HIC_{15}');
%         Ylabel='Operational velocity (m/s)';
%         figName = 'SimE-ScatterPlot-HIC-Child-head';
%         set(gcf, 'name', figName);
