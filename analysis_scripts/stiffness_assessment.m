...
%     vel_samples = [1.0:1:10.0];
    ...
    contact_stiffness = linspace(0.002,1.0,15);
    contact_stiffness = [contact_stiffness, 1.5, 2.0].*1e09;
    X_variable = contact_stiffness./1e9;
    Variable2_label = 'Contact Stiffness (1e^9 N/m)';
...
%         qolo_robot.elastic_mod_robot_cover = material_modulus(k_material);
        robot.stiffness_kcb = contact_stiffness(k_material);
...
%                 find(service_test{j_test}.data.time>minTime,1):find(service_test{j_test}.data.time>maxTime,1);
    ...
    %% Head Impact Simulations: 
       ...
       labelOrientation = 'vertical';
        figName = 'SimK-Force_peak-child-head';
%         plegends = material_names;
        ...
        nfig = nfig+1;
        figName = 'SimK-Acceleration-peak-child-head';
        minTime = 0;
        maxTime = method.time;
        AxisPlots = [minTime maxTime -100 7000];
        plotNpairedData(nfig,[operational_vel',... 
...
%         contourf(X_variable,operational_vel,Zcontour,'--');
        ...
        figName = 'SimK-Heatmap-Max-acceleration-Child-head';
...
%         contourf(X_variable,operational_vel,Zcontour,'--');
        ...
        Ylabel='Operational velocity (m/s)';
        figName = 'SimK-Heatmap-HIC-Child-head';
        set(gcf, 'name', figName);
        set(gcf, 'Position', [10 10 880 1080]);
        set(gca,'FontName',Fonts,...
                'FontSize', FontSizes,...
...
%         set(get(cb,'label'),'string','HIC_{15}');
%         Ylabel='Operational velocity (m/s)';
%         figName = 'SimK-ScatterPlot-HIC-Child-head';
%         set(gcf, 'name', figName);
%         set(gcf, 'Position', [10 10 880 1080]);
%         set(gca,'FontName',Fonts,...
%                 'FontSize', FontSizes,...
