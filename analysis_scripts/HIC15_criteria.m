...
% 
% Inputs:
%           Acc  :      [REQ'D]    [m x 1]{double};   Column-wise vector of acceleration data in [g = 9.81m/s^2]
%           time    :   [REQ'D]    [m x 1]{double};   Column-wise vector of sampling times in [s]
...
function [hic_value,interval] = HIC15_criteria(time,Acc,varargin)
   ...
   % =============== Plots setup ===================% 
     if p.Results.ShowPlots
            minY = min(Acc);
            maxY = max(Acc)*1.1;
            minX = interval(1)-maxWindow*0.05;
